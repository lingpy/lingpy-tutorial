...
"""LingPy tutorial command line script."""
...
def errors():
    def check_sequence(seq):
        ...
        # doesn't matter which model to take, all cover the same character range
        cls = tokens2class(seq, 'dolgo')
...
def wordlist2():
    ...
    wl = Wordlist('polynesian.tsv')
    # get all indices for concept "hand", `row` refers to the concepts here,
    # while `col` refers to languages
    eight = wl.get_dict(row='Eight', entry='value')
    for taxon in ['Emae_1030', 'RennellBellona_206', 'Tuvalu_753',
                  'Sikaiana_243', 'Penrhyn_235', 'Kapingamarangi_217']:
...
def alignments3():
    ...
    msa.prog_align()
    print(msa)
    print('There is {0} swap in the alignment.'.format(
        'no' if not msa.swap_check(swap_penalty=-1) else 'a'))
...
def cognates_turchin():
    ...
    for k, v in eight.items():
        ...
        print("{0:20} \t {1} \t{2}".format(
            lex[idx, 'doculect'], lex[idx, 'value'], lex[idx, 'turchinid']))
